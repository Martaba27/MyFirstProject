...
public class ReadMe {
  ...
  /*  Duo tech BMA(Bank Account Management Application)

    In this OOP project, you will be creating an application that loads the data from an external file and generates customer bank accounts based on the loaded data. You will be utilizing OOP principles, Exceptions and File I/O to achieve the given task.
    ...
    a.BaseInterestRate interface - An interface that represents the behaviors common for all bank accounts.
            ...
            * BASE_INTEREST_RATE - a constant that represents  the base interest rate for all bank account types set by the Federal Reserve. Must be initialized to 2.5.
    ...
    Fields(protected):
            ...
            * name - represents an account holder's full name
            * ssn - represents an account holder's ssn
   ...
   * initializes routingNumber by calling generateRoutingNumber() method
	 ...
	 * generateRoutingNumber() - generates a 9-digit Routing  number using the below formula:
   ...
   * showInfo() - Overrides super class's showInfo() method by ADDING Checking account specific info
    ...
    Fields:
            ...
            * safetyDepositBoxID - represents a safe deposit box id
   * safetyDepositBoxPin - represents a safe deposit box pin
    ...
    BALANCE: 1000.0
    ACCOUNT TYPE: Checking
    SAFE DEPOSIT BOX ID: 793
    SAFE DEPOSIT BOX PIN: 9877
    INTEREST RATE: 2.25%
    ...
    Uses File class to read from a file and handles the checked exception with a regular try-catch block or try-with-resources block.


            2.Implement the application logic in the main method:

    Your application should do the following:
            1. Asks the user to enter the name of the csv file. (3 different csv files have been added to the project.)
